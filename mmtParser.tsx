...
/*if (!domNode.parent) {
      (domNode as any).children[1].children = (domNode as any).children[1].children.slice(0, 200);
      console.log(domNode);
    }*/
const replace = (domNode: DOMNode, skipSidebar = false) => {
  ...
  if (domNode.attribs?.class === "sidebar") {
    ...
    return (
      <>
        <Box height="0px" maxWidth="300px" display={{ xs: "none", md: "block" }}>
        ...
        <Box height="0px" display={{ xs: "block", md: "none" }}>
          <div className="sidebarbuttonwrapper" style={{ marginTop: `${shift}px` }}>
            <ButtonAndDialog content={domToReact(domNode.children, { replace })} />
          </div>
        </Box>
      </>
    );
  ...
  if (previous?.attribs?.class === "sidebar" && domNode.attribs?.class !== "sidebar") {
          ...
          <NoMaxWidthTooltip
            title={
              <div style={{ minWidth: "300px", maxWidth: "600px" }}>
                <ContentFromUrl url={tooltipPath} modifyRendered={(n) => getChildrenOfBodyNode(n)} />
              </div>
            }
          >
