...
export default async function handler(
  ...
  if (toBeAdded) {
    ...
    if (
      !(await isCurrentUserMemberOfAClupdater(aclId, res, req)) &&
      memberId != userId
    )
      return res.status(403).end();
