...
export async function checkJobApplicationExists(
    id: number,
    res: NextApiResponse
  ){
    const results = await executeDontEndSet500OnError(
      'SELECT * FROM jobapplication WHERE jobPostId = ?',
      [id],
      res
    );
...
const jobApplicationExists = await checkJobApplicationExists(jobPostId,res);
