...
import { Button, IconButton, Box, TableContainer, Paper, Table, TableHead, TableRow, TableCell, TableBody, Chip, Tooltip } from "@mui/material";
import { Search as SearchIcon, Phone as PhoneIcon, Mail as MailIcon, ExpandLess, ExpandMore, CheckCircle, Cancel, PendingActions } from "@mui/icons-material";
import { getJobApplicationsByUserId, getJobPostById, getOrganizationProfile, updateJobApplication } from '@stex-react/api';
...
const Applications = () => {
  ...
  const handleAcceptOffer = async (job) => {
    await updateJobApplication({ ...job, applicantAction: "ACCEPT_OFFER", applicationStatus: "OFFER_ACCEPTED" });
    setAppliedJobs((prevJobs) =>
      prevJobs.map((j) => (j.id === job.id ? { ...j, applicantAction: "ACCEPT_OFFER", applicationStatus: "OFFER_ACCEPTED" } : j))
    );
  };
  ...
  return (
      <Box padding={2}>
        <Box display="flex" justifyContent="center" gap={2} mb={2}>
          ...
          <Button variant={filter === "ON HOLD" ? "contained" : "outlined"} onClick={() => setFilter("ON HOLD")}>ON HOLD</Button>
          <Button variant={filter === "ACCEPTED" ? "contained" : "outlined"} onClick={() => setFilter("ACCEPTED")}>ACCEPTED</Button>
        </Box>
                ...
                <TableRow key={jobApplication.id} hover>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Tooltip title={jobApplication.applicationStatus !== "OFFERED" ? 'Action not available until an offer is made' : 'Accept the offer'}>
                        <span>
                          <Button
                            variant="contained"
                            color="success"
                          ...
                          >
                            {jobApplication.applicantAction === "ACCEPT_OFFER" ? "Offer Accepted" : "Accept Offer"}
                          </Button>
                        </span>
                      </Tooltip>
                      <Tooltip title={jobApplication.applicationStatus !== "OFFERED" ? 'Action not available until an offer is made' : 'Reject the offer'}>
                        <span>
                          <Button
                            variant="outlined"
                            color="error"
                            ...
                            disabled={jobApplication.applicationStatus !== "OFFERED" || jobApplication.applicantAction === "ACCEPT_OFFER" || jobApplication.applicantAction === "REJECT_OFFER"}
                          >
                            {jobApplication.applicantAction === "REJECT_OFFER" ? "Offer Rejected" : "Reject Offer"}      
                          </Button>
                        </span>
                      </Tooltip>
                    </Box>
...
const Application = () => {
    return <JpLayoutWithSidebar role="student">
        {<Applications />}
        </JpLayoutWithSidebar>;
};
