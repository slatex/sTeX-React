/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_solutions_free: (a: number, b: number) => void;
export const solutions_from_json: (a: number, b: number) => number;
export const solutions_to_json: (a: number, b: number) => void;
export const solutions_check_response: (a: number, b: number) => number;
export const __wbg_exercisefeedback_free: (a: number, b: number) => void;
export const __wbg_get_exercisefeedback_correct: (a: number) => number;
export const __wbg_set_exercisefeedback_correct: (a: number, b: number) => void;
export const exercisefeedback_from_json: (a: number, b: number) => number;
export const exercisefeedback_to_json: (a: number, b: number) => void;
export const set_debug_log: () => void;
export const render_document: (a: number, b: number, c: number, d: number, e: number, f: number) => void;
export const render_fragment_with_cont: (a: number, b: number, c: number, d: number, e: number) => void;
export const render_fragment: (a: number, b: number, c: number, d: number, e: number) => void;
export const ftml_setup: (a: number, b: number) => number;
export const __wbg_ftmlmounthandle_free: (a: number, b: number) => void;
export const ftmlmounthandle_unmount: (a: number) => void;
export const get_document_html: (a: number, b: number) => number;
export const get_paragraph_html: (a: number, b: number) => number;
export const set_server_url: (a: number, b: number) => void;
export const get_server_url: (a: number) => void;
export const __wbg_leptoscontext_free: (a: number, b: number) => void;
export const leptoscontext_cleanup: (a: number) => void;
export const leptoscontext_wasm_clone: (a: number) => number;
export const __wbg_intounderlyingsource_free: (a: number, b: number) => void;
export const intounderlyingsource_pull: (a: number, b: number) => number;
export const intounderlyingsource_cancel: (a: number) => void;
export const __wbg_intounderlyingsink_free: (a: number, b: number) => void;
export const intounderlyingsink_write: (a: number, b: number) => number;
export const intounderlyingsink_close: (a: number) => number;
export const intounderlyingsink_abort: (a: number, b: number) => number;
export const __wbg_intounderlyingbytesource_free: (a: number, b: number) => void;
export const intounderlyingbytesource_type: (a: number) => number;
export const intounderlyingbytesource_autoAllocateChunkSize: (a: number) => number;
export const intounderlyingbytesource_start: (a: number, b: number) => void;
export const intounderlyingbytesource_pull: (a: number, b: number) => number;
export const intounderlyingbytesource_cancel: (a: number) => void;
export const __wbindgen_export_0: (a: number, b: number) => number;
export const __wbindgen_export_1: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_export_2: (a: number) => void;
export const __wbindgen_export_3: (a: number, b: number, c: number) => void;
export const __wbindgen_export_4: WebAssembly.Table;
export const __wbindgen_add_to_stack_pointer: (a: number) => number;
export const __wbindgen_export_5: (a: number, b: number) => void;
export const __wbindgen_export_6: (a: number, b: number, c: number) => void;
export const __wbindgen_export_7: (a: number, b: number, c: number) => number;
export const __wbindgen_export_8: (a: number, b: number, c: number, d: number) => void;
